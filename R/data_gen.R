...
.construct_pileupStudy <- function(Gene, regions, Ranges, BAMfiles, caseIDs, max_depth=100000, strand.specific=FALSE, nCores=2) {
    ...
    pileup <- do.call(cbind, BiocParallel::bplapply(
        seq_along(BAMfiles),
        FUN=function(i) {
            bam <- BAMfiles[i]
            message("Reading BAM: ", bam)
            read_aBAM(BAM=bam, regions=new.regions, strand.specific=strand.specific, max_depth=max_depth)
        },
...
filter_lowExpGenes <- function(genelist, TPM, thr=5, pct=40) {
    ...
    rows_to_keep <- apply(TPM2, 1, function(row) {
        mean(row < thr) < pct / 100
    })
