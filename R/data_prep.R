...
norm_pileup.spl <- function(pileup, rnum=100, method=1) {
    ...
    if (method == 1) {
        ## Method 1: Raw value
        ## Find read depth at even points (green points)
        readdepth <- depthmat[pos3, 2]
    } else if (method == 2) {
        ## Method 2: Interpolation
        ## Find geometric mean (blue points) using read depth at odd points (red points)
        readdepth <- 10^(round(zoo::rollmean(log10(depthmat[pos4, 2] + 1), 2), 10)) - 1
    } else {
...
norm_pileup.gene <- function(pileupData, rnum=100, method=1, nCores=2) {
    if (!(method %in% c(1, 2))) {
        stop(method, " is not an option for method.")
    }
    ...
    }, BPPARAM=BiocParallel::MulticoreParam(workers=max(1L, as.integer(floor(nCores / 2))))))
...
norm_pileup.list <- function(pileupPath, geneNames=NULL, rnum=100, method=1, cases=NULL) {
    if (is.null(geneNames)) {
        geneNames <- paste0("Gene_", seq_along(pileupPath))
    }
    ...
    if (length(pileupPath) != length(geneNames)) {
        ...
        pileupList <- list()
        pileupList <- lapply(seq_along(pileupPath), function(g) {
            get_pileupExon(g, pileupPath, cases)
        })
