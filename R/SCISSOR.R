...
## Converted to Bioconductor coding style from the original R code (comments):
...
get_Ranges <- function(Gene=NULL, regions=NULL, GTF.file=NULL, hg.ref=c("hg19", "hg38"), outputType="part_intron") {
    ...
    if (strnd == "+") {
        ...
        gRanges <- matrix(gRanges0[rev(seq_len(nrow(gRanges0))), rev(seq_len(4))], ncol=4)
        lRanges <- matrix(max(lRanges0) - lRanges0[rev(seq_len(nrow(lRanges0))), rev(seq_len(4))] + 1, ncol=4)
        cRanges <- matrix(max(cRanges0) - cRanges0[rev(seq_len(nrow(cRanges0))), 2:1] + 1, ncol=2)
    }
    rm(gRanges0, lRanges0, cRanges0)
    colnames(gRanges) <- c("ip.start", "e.start", "e.end", "ip.end") # e:exon; ip: intronic part;
    rownames(gRanges) <- paste("exon", seq_len(nrow(gRanges)), sep="")
    colnames(lRanges) <- c("ip.start", "e.start", "e.end", "ip.end")
    rownames(lRanges) <- paste("exon", seq_len(nrow(gRanges)), sep="")
    colnames(cRanges) <- c("e.start", "e.end")
    rownames(cRanges) <- paste("exon", seq_len(nrow(gRanges)), sep="")
...
find.exon.hy <- function(exon, is.intron=FALSE, num.intron=NULL) {
    ...
    if (!is.intron) {
        ...
        # }
        ## Vectorize from for loop
        coverage.col <- unlist(lapply(seq_len(nrow(ep2)), function(iep) {
            ep2[iep, 1]:ep2[iep, 2]
        }))
        ...
        if (is.null(num.intron)) {
            ...
            ep2 <- cbind(ep2[, 1] - num.intron, ep2, ep2[, 2] + num.intron)
            ep2[1, 1] <- 1
            if (nrow(ep2) > 1) {
...
build_pileup <- function(Pileup, caseIDs=NULL, regions, inputType="part_intron", outputType="part_intron") {
    ...
    if (strnd == "-") {
        ...
        rawPileup <- Pileup[rev(seq_len(nrow(Pileup))), ]
    } else {
        rawPileup <- Pileup
    }
    if (is.null(caseIDs)) {
        caseIDs <- paste0("case-", seq_len(ncol(rawPileup)), sep="")
    }
    if (inputType == "whole_intron") {
        ...
        if (outputType == "whole_intron") {
            ...
            # }
            ## Vectorize from for loop
            exonic.region <- unlist(lapply(seq_len(nrow(ep.new.temp$ep)), function(i) {
              ep.new.temp$epl[i]:ep.new.temp$epr[i]
            }))
    ...
    strtend.num <- matrix(as.numeric(strtend), ncol=2)
    allPos <- unlist(sapply(seq_len(nrow(strtend.num)), function(x) strtend.num[x, 1]:strtend.num[x, 2]))
    ...
    if (strnd == "+") {
        ...
        return(covPileup[rev(seq_len(nrow(covPileup))), ])
    }
}
...
process_pileup <- function(pileupData, Ranges, logshiftVal=NULL, plotNormalization=FALSE, ...) {
    ## Log transform data
    data.log <- logtransform_data(inputData=pileupData, logshiftVal=logshiftVal)

    ## Center and normalize data
    data.normalized <- normalize_data(
        inputData  = data.log$outputData,
    ...
    intron.rm <- c()
    if (nrow(exonset) > 1) {
        ## for loop retained intentionally due to sequential dependency
        ...
        for (ie in 1:(nrow(exonset) - 1)) {
            ...
            q.exon <- apply(cbind(
                apply(pileupData[b.exon1, ], 2, FUN=function(x) {
                    quantile(x, probs=0.5)
                }),
                apply(pileupData[b.exon2, ], 2, FUN=function(x) {
                    quantile(x, probs=0.5)
                })
            ), 1, max)
            ...
            if (lcarea < 22) { # remove introns less than 30 (22+4+4)
                ...
                q.intron <- apply(cbind(
                    apply(pileupData[b.intron[6:ceiling(lcarea / 3)], ], 2, FUN=function(x) {
                        quantile(x, probs=0.75)
                    }),
                    apply(pileupData[b.intron[ceiling(lcarea / 3):(2 * ceiling(lcarea / 3))], ], 2, FUN=function(x) {
                        quantile(x, probs=0.75)
                    }),
                    apply(pileupData[b.intron[(2 * ceiling(lcarea / 3)):(lcarea - 6)], ], 2, FUN=function(x) {
                        quantile(x, probs=0.75)
                    })
                ), 1, max)
        ...
        ## for loop retained intentionally due to sequential dependency
        new.exonset <- exonset # pre-allocate
        for (ie in 1:(nrow(exonset) - 1)) {
        ...
        ## Vectorize from for loop
        if (length(exon.rm) > 0) {
            rows_to_zero <- unlist(lapply(exon.rm, function(ie) {
                new.exonset[ie, 1]:new.exonset[ie, 4]
            }))
            data.normalized$outputData[rows_to_zero, ] <- 0
        }
        ...
        ## Vectorize from for loop
        if (length(intron.rm) > 0) {
            rows_to_zero <- unlist(lapply(intron.rm, function(ie) {
                (exonset[ie, 3] + 1):(exonset[ie + 1, 2] - 1)
            }))
            data.normalized$outputData[rows_to_zero, ] <- 0
        }
    }
    ...
    return(list(
        ...
        g1.offset      = data.normalized$g1.offset, 
        g2.offset      = data.normalized$g2.offset
    ))
}
...
getShiftParam <- function(X, param.grid=NULL, draw.plot=FALSE) {
    ...
    res <- apply(matrix(param.grid, ncol=1), 1,
        FUN=function(x) {
            SlogADstat.hy(X=X, shift.val=x)$ADval
        }
    )
    ...
    if (draw.plot) {
        ...
        text(
            x = optim.param + (0.2 * (max(param.grid) - min(param.grid))),
...
scale.factor <- function(X, average="mean", trim=0.1, adjval=NULL, robustLM=FALSE) {
    mean.vec <- apply(X, 1, FUN=function(x) {
        adj.center(x, average=average, trim=trim, adjval=adjval)
    })
    if (sum(mean.vec^2) > 0) {
        if (robustLM) {
            msf <- apply(X, 2, FUN=function(x) {
                MASS::rlm(x ~ mean.vec - 1, psi=MASS::psi.bisquare)$coefficients
            })
...
normalize_data <- function(inputData, pileupData, Ranges, smoothness=0.7, makePlot=FALSE, ...) {
    centerDataResult <- center_data(inputData=inputData, Ranges=Ranges)
    exonset <- Ranges$lRanges
    Gene <- Ranges$Gene
    g1.offset <- estimate_offset(
        centerDataResult = centerDataResult,
        msf              = NULL, 
        centeredData     = NULL,
        pileupData       = pileupData,
        exonset          = exonset,
    ...
    if (sum((g1.offset$g - 1)^2) < 1e-10) {
        ...
        while ((k < 5) & (max(g2.offset$g) > 1.05)) {
            g2.offset <- estimate_offset(
                centerDataResult = NULL,
                msf              = msf, 
                centeredData     = normalizedData,
                pileupData       = pileupData,
                exonset          = exonset,
                smoothness       = smoothness, 
                makePlot         = FALSE
...
center_data <- function(inputData, Ranges, average="mean", trim=0.1, adjval=NULL, ...) {
    ...
    mean.vec <- apply(inputData, 1, FUN=function(x) {
        adj.center(x, average=average, trim=trim, adjval=adjval)
    })
    ...
    # }
    ## Vectorize from for loop
    zeromean <- unlist(lapply(seq_len(nexons), function(ie) {
        x <- mean.vec[Ranges$lRanges[ie, 2]:Ranges$lRanges[ie, 3]]
        if (sum(x^2) > 1e-05) {
            exonbaseMat[Ranges$lRanges[ie, 2]:Ranges$lRanges[ie, 3], ie] <<- x / sum(x^2)
            NULL
    ...
    keepexons <- which(!seq_len(nrow(msf.exons)) %in% zeromean)
    if (length(keepexons) > 1) {
        msf <- apply(msf.exons[which(!seq_len(nrow(msf.exons)) %in% zeromean), ], 2, median)
    } else if (length(keepexons) == 1) {
        # msf <- msf.exons[which(!c(1:nrow(msf.exons)) %in% zeromean), ]
        msf <- msf.exons[which(!seq_len(nrow(msf.exons)) %in% zeromean), ]
    } else {
...
estimate_offset <- function(
    ...
    # }
    ## Vectorize from for loop
    exonbase <- unlist(lapply(seq_len(nrow(exonset)), function(j) {
      exonset[j, 2]:exonset[j, 3]
    }))
    ...
    if (length(goodcase) > floor(0.1 * n)) {
        ...
        if (makePlot) {
            ...
            plot(msf, case.sse.adj,
                col  = colmat,
                xlab = "median scale factor", 
                ylab = "variability",
...
interpolate.hy <- function(x, y, newdata) {
    ...
    return(apply(matrix(newdata, ncol=1), 1, FUN=function(z) {
        inner.fn(x=x, y=y, newx=z)
    }))
}
...
plot_offset <- function(offset.obj, draw.legend=TRUE, colmat=NULL, ...) {
    ...
    sse <- offset.obj$sse
    lowess.fit <- offset.obj$lowess.fit
    smoothness <- offset.obj$smoothness
    ...
    if (draw.legend) {
        plot(
            ...
            sse,
            col  = colmat, 
            ylim = c(yaxis[1], yaxis[2] + 0.2 * (yaxis[2] - yaxis[1])),
            xlab = "median scale factor", 
            ylab = "variation level", 
            ...
        )
    } else {
        plot(
...
decay.rate.hy <- function(Data, dist2end=NULL, robust=FALSE) {
    ...
    # }
    ## Vectorize from for loop
    lmcoef <- t(vapply(seq_len(n), function(case) {
        coef(lm(Data[, case] ~ x))
    }, numeric(2)))
    rate <- lmcoef[, 2] * d
...
compScales <- function(x, rmZeroes=FALSE, maxRatio=NULL, precScale=1e-10) {
