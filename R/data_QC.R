...
get_DR <- function(genelist, pileupPath, sampleInfo, cases=NULL, nCores=2) {
...
get_DIIhc <- function(DR, topPct=5) {
    ...
    cluster_means <- hd.vec %>%
        ...
        ## Degraded if meanSum is greater; Degraded: Cluster=1
        mutate(DII=c("Degraded", "Intact"), Cluster_new=seq_len(2))
...
compute_DIIwt <- function(DR, alpha=2, cutoff=3, TPM, thr=5, pct=40, genelength) {
    ...
    ds.vec <- data.frame(
        ...
        mutate(
            PD  = pd.rate.hy(DS, qrsc=TRUE), # projection depth
...
get_MCD <- function(genelist, pileupPath, sampleInfo, cases=NULL, nCores=2) {
    cl <- parallel::makeCluster(nCores)
    doParallel::registerDoParallel(cl)
    on.exit(parallel::stopCluster(cl), add=TRUE)
...
get_wCV <- function(genelist, pileupPath, sampleInfo, rnum=100, method=1, winSize=20, egPct=10, cases=NULL, nCores=2) {
...
compute_SOI <- function(MCD, wCV, rstPct=20, obsPct=50, cutoff=3) {
    ...
    auc.coord <- stats::na.omit(data.frame(
        Gene   = rep(rownames(MCD), ncol(MCD)),
    ...
    auc.vec <- smoothData %>%
        ...
        mutate(
            PD  = pd.rate.hy(AUC, qrsc=TRUE), # projection depth
