...
plot_DIIwt <- function(DR, DIIresult, cutoff=3, outFile=NULL) {
    ...
    df <- data.frame(
        var   = rep(colnames(ds.vec2), each=nrow(ds.vec2)),
    ...
    ecdf_list <- lapply(seq_len(nrow(mt)), function(i) {
        convert_Cont2eCDF(mt[i, ], rownames(mt)[i], margin=1)[, 2]
    })
    ecdf <- do.call(cbind, ecdf_list)
    rownames(ecdf) <- colnames(mt)
    ...
    gene.df <- gene.df %>%
        mutate(merged_kb_ecdf=ecdf$merged_kb, mean.TPM_ecdf=ecdf$mean.TPM)

    col_fun_prop3 <- colorRamp2(c(0, 0.5, 1), c(colwheel[2], "white", colwheel[4]))
    lgd3 <- Legend(
        col_fun        = col_fun_prop3, 
        title          = "Gene length", 
        ...
        labels         = c(
            round(min(gene.df$merged_kb), 1), 
            round(quantile(gene.df$merged_kb, 0.5), 1), 
            round(max(gene.df$merged_kb), 1)
        )
    )

    col_fun_prop4 <- colorRamp2(c(0, 0.5, 1), c(colwheel[2], "white", colwheel[4]))
    lgd4 <- Legend(
        col_fun        = col_fun_prop4, 
        title          = "Mean TPM", 
        title_position = "topcenter", 
        at             = c(0, 0.5, 1), 
        direction      = "horizontal", 
        ...
        labels_gp      = gpar(fontsize=8),
        labels         = c(
            round(min(gene.df$mean.TPM), 1), 
            round(quantile(gene.df$mean.TPM, 0.5), 1), 
            round(max(gene.df$mean.TPM), 1)
        )
    )
    ...
    row_ha <- rowAnnotation(
        ...
        col                 = list("Gene length"=col_fun_prop3, "Mean TPM"=col_fun_prop4),
        show_legend         = FALSE,
        gap                 = unit(2, "mm")
    )
    ...
    p <- grid.grabExpr({
        ...
        draw(hm.degrate)
        pd <- packLegend(
            list      = lapply(c(0, 3:4), function(i) get(paste0("lgd", i))),
...
plot_SOI <- function(SOIresult, cutoff=3, outFile=NULL) {
    ...
    df <- data.frame(
        var   = rep(colnames(auc.vec2), each=nrow(auc.vec2)),
    ...
    p <- ggplot2::ggplot(auc.coord, aes(x=x, y=y, group=Sample, color=SOI)) +
        ...
        geom_rect(
            data        = tibble::tibble(x1=rangeMCD[1], x2=rangeMCD[2], y1=-Inf, y2=+Inf),
            inherit.aes = FALSE,
        ...
        xlab(expression(paste(log[10], "(MCD+1)"))) +
        ylab("wCV") +
